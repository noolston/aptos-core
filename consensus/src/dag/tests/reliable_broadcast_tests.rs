...
use crate::{
    dag::{
        ...
        dag_store::Dag,
        reliable_broadcast::{BroadcastStatus, NodeBroadcastHandler, ReliableBroadcast, NodeBroadcastHandleError},
...
async fn test_node_broadcast_receiver_failure() {
    ...
    let mut rb_receivers: Vec<NodeBroadcastHandler> = signers.iter().map(|signer| {
    ...
    let partial_sigs = PartialSignatures::new(BTreeMap::from([(signers[1].author(), node_sig.signature().clone())]));
    let node_cert = NodeCertificate::new(node.metadata().clone(), validator_verifier.aggregate_signatures(&partial_sigs).unwrap());
    let node = new_node(1, 20, signers[0].author(), vec![node_cert]);
    assert_eq!(rb_receivers[1].process(node.clone()).unwrap_err().to_string(), "not enough voting power");    

    // Round 0 - add all nodes
    let node_certificates: Vec<_> = signers.iter().map(|signer| {
        ...
        NodeCertificate::new(node.metadata().clone(), validator_verifier.aggregate_signatures(&partial_sigs).unwrap())
    }).collect();

    // Add Round 1 node with proper certificates
    let node = new_node(1, 20, signers[0].author(), node_certificates);
    assert_eq!(rb_receivers[0].process(node).unwrap_err().to_string(), NodeBroadcastHandleError::MissingParents.to_string());
